...
**Decision** (resolved by spec.md's Clarification #1): Change `events.start_date` from `DATE` to `DATETIME`. The EventResource form exposes **one** field ("date/time") bound to `start_date`. `end_date` remains a `DATE` column, is **not** exposed on the form, and is dehydrated automatically to `start_date`'s calendar date on every save **made through this feature's Filament form** (a `mutateFormDataBeforeCreate()`/`mutateFormDataBeforeSave()` hook on the EventResource's pages — not a model-level `Event::saving()` hook). The existing CHECK constraint (`end_date >= start_date AND end_date <= DATE_ADD(start_date, INTERVAL 1 DAY)`, feature 001) is updated to compare against `DATE(start_date)` so it continues to hold trivially for events created through this admin panel.

**Correction discovered during implementation (2026-08-14)**: an earlier version of this decision put the derivation on `Event::booted()`'s `saving` event, i.e. every save through the model, not just Filament's. That breaks feature 001's already-shipped, already-tested multi-day event support — `tests/Feature/Schema/EventDurationBoundsTest.php` and `EventMultiDayLookupTest.php` directly construct events with `end_date` one day after `start_date` and assert the CHECK constraint's exact >1-day boundary via `Event::factory()->create([...])`, which calls `Event::save()` directly, bypassing any Filament form. A model-global hook would silently collapse every such event back to single-day and make those tests fail. The fix: scope the derivation to the Filament form layer only. Events created or edited outside this feature (feature 001's factories/tests, or any future non-Filament code) keep full control over `end_date`, preserving multi-day events as a capability of the schema itself — this feature's admin UI simply chooses not to expose that control to staff.
