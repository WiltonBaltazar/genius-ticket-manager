...
## 3. Event date/time: time-of-day precision on `start_date`; `end_date` staff-editable for multi-day events

**Original decision** (resolved by spec.md's Clarification #1): Change `events.start_date` from `DATE` to `DATETIME`. The EventResource form initially exposed **one** field ("date/time") bound to `start_date`, with `end_date` dehydrated automatically to `start_date`'s calendar date on every save **made through this feature's Filament form** (a `mutateFormDataBeforeCreate()`/`mutateFormDataBeforeSave()` hook on the EventResource's pages — not a model-level `Event::saving()` hook, since that would break feature 001's already-shipped multi-day event tests — see the 2026-08-14 correction note below, still applicable).

**Superseded 2026-08-14 (post-implementation change request)**: events must now support spanning more than one day. The EventResource form gained an optional `end_date` `DatePicker` ("End Date", date-only, `afterOrEqual('start_date')`). The `mutateFormDataBeforeCreate()`/`mutateFormDataBeforeSave()` hooks changed from unconditionally overwriting `end_date` to only defaulting it (`$data['end_date'] ??= ...`) when staff leave the field blank — so single-day events still need zero extra input, but staff can now explicitly set a later end date. The `events_duration_check` CHECK constraint's one-day upper bound was dropped in a further migration (`2026_08_14_170142_allow_multi_day_events.php`), leaving only `end_date >= DATE(start_date)`.

**Correction discovered during initial implementation (2026-08-14, still applicable)**: an earlier version of the original decision put the derivation on `Event::booted()`'s `saving` event, i.e. every save through the model, not just Filament's. That would have broken feature 001's already-shipped, already-tested multi-day event support at the model layer — `tests/Feature/Schema/EventDurationBoundsTest.php` and `EventMultiDayLookupTest.php` directly construct multi-day events via `Event::factory()->create([...])`, bypassing any Filament form. The fix — scoping the derivation to the Filament form layer only — remains correct and is now moot for the duration *cap* specifically (removed), but still matters for the *default-when-blank* behavior: feature 001's direct-model event creation must keep full, unmediated control over `end_date`, independent of whatever default this admin panel's form applies to its own blank-field case.

**Rationale**: The admin UI should not force every single-day event creator (the common case) to fill in a redundant end date, but must let staff who need a multi-day event set one directly rather than inferring it some other way. Defaulting-when-blank plus a relaxed DB constraint satisfies both with minimal staff friction and no schema redesign.

**Alternatives considered**: Requiring `end_date` on every save (rejected — adds friction to the common single-day case for no benefit). Capping multi-day span at some arbitrary maximum (rejected — the change request didn't ask for a cap, and inventing one would be an unrequested constraint).
