...
### Post-implementation change (2026-08-14)

- Q: Events were built as single-day-only (end_date auto-derived from start_date, matching the original request's single "date/time" field). The request has changed — events must now be able to span more than one day. → A: Add an optional End Date field to the EventResource form (date-only, defaulting to the start date's calendar day when left blank, so single-day events need no extra input); staff can set a later end date for multi-day events. The `events_duration_check` CHECK constraint's one-day cap (feature 001) is removed — only `end_date >= start_date`'s calendar date is still enforced.
...
- **FR-004**: The system MUST enforce four staff roles with distinct permissions across events, ticket types, and orders: super admin (every action these requirements define for the three resources below, including deleting events, ticket types, and orders — super admin is the only role that can delete any of these three resources), event manager (create/edit events and ticket types, view orders, cannot delete events or ticket types), support (view orders only), and gate operator (no access to events, ticket types, or orders management). A staff account whose role does not match one of these four values (including no role at all) MUST be treated as having the same access as gate operator — no access to events, ticket types, or orders.
...
|---|---|---|---|
| Super Admin | Create, view, edit, delete | Create, view, edit, delete (FR-011) | View, delete (FR-028); no field edits (read-mostly, FR-016) |
| Event Manager | Create, view, edit (no delete, FR-010) | Create, view, edit (no delete, FR-011) | View only (no delete, FR-028) |
| Support | No access | No access | View only (no delete, FR-028) |
| Gate Operator | No access | No access | No access |

No role, including super admin, can edit any order field through this panel — that is a feature-wide constraint (FR-016), not a role-specific gap. Ticket type and order deletion, unlike field edits, are each restricted to the super admin role specifically (FR-011, FR-028) — the same restriction FR-010 already applies to event deletion.
- **FR-006**: The system MUST allow permitted staff to create and edit events with: name, a unique slug, location, a start date and time (time-of-day precision, not date-only), an optional end date (date-only; defaults to the start date's calendar day when left blank, so an event spans more than one day only when staff explicitly set a later end date), an optional hero image, a formatted description (optional, consistent with the already-migrated `events.description` column being nullable per feature 001's schema), a status (draft, published, closed, or archived), and internal notes visible only to staff. "Staff" here means any role with view/edit access to that event under the Role Permission Matrix (super admin and event manager) — support and gate operator have no events access at all, so the notes-visibility question doesn't arise for them. A hero image is never required to save an event, at any status, and may be added or changed later. Staff may set an event's status to any of the four values at any time, in any order — no transition is restricted.
...
- **FR-028**: The system MUST restrict order deletion to the super admin role; event manager and support can view orders but not delete them. This is a deletion of the order record itself, distinct from FR-016's prohibition on editing an order's fields.
...
- **Event**: A ticketed event staff manage — name, unique slug, location, start date and time (time-of-day precision), optional end date (may span more than one day), optional hero image, optional description, status (draft/published/closed/archived), and internal notes visible only to staff with events access. Owns one or more ticket types.
...
- "Location" refers to the event's existing venue information already tracked by the system; this feature exposes the field staff need to manage that information rather than introducing a new venue model. The event's start date/time gains time-of-day precision as resolved in Clarifications; the exact column-level representation is a planning-level decision. As of the 2026-08-14 post-implementation change, the end date is staff-editable (optional, defaulting to the start date's calendar day) rather than always auto-derived — events may span more than one day.
