...
- [X] T049 [P] [US3] Pest test: inserting a second order with a duplicate `stripe_payment_intent_id` throws a unique-constraint violation in `tests/Feature/Schema/OrderPaymentIntentUniquenessTest.php`
...
- [X] T061 [P] Pest test verifying the index groups from data-model.md exist — `events(status, start_date)`, `ticket_types(event_id)`, `orders(attendee_id, status)`, `orders(attendee_id, created_at DESC)` (constitution Principle IV, added per plan.md's Complexity Tracking), `orders(stripe_payment_intent_id)` unique, `tickets(qr_code)` unique — via `information_schema.statistics` in `tests/Feature/Schema/IndexPresenceTest.php`
...
- [X] T066 [P] Pest test: order lookup by `attendee_id`, by `status`, and by `stripe_payment_intent_id` each return the correct row(s) (FR-019); combined with an `EXPLAIN`/timing check against a seeded representative row count to validate SC-006 in `tests/Feature/Schema/OrderLookupPerformanceTest.php`
